...
const Response = () => {
  ...
  return (
    <div className="response-container bg-black p-0 m-0 min-w-96 min-h-64 overflow-x-hidden" >
      ...
      <div className="text-white text-4xl font-bold bg-black/30 p-4  text-left z-20 fixed" style={{width:"100%"}}>
      ...
      {messages.map((ele, index) => (
        <>
          <div
            className={`message ${index % 2 == 0 ? "message-right" : "message-left"} min-w-56 dark:bg-zinc-700 mb-4 min-h-fit relative `}
            key={index}
          >
            {index % 2 != 0 ? <AiIcon /> : ""}
            {ele[0]}
